...
or manually drawn route. Results are shown on a Mapbox map and summarized in a
location report.
...
| `nextjs/app/lib/query-definition.ts`    | Versioned bookmarkable-query contract and validation             |
| `nextjs/app/lib/query-url.ts`           | Query URL parsing, legacy adaptation, and canonical serialization |
| `nextjs/app/lib/drawn-route-codec.ts`   | Precision-6 GeoJSON/polyline conversion and size defenses        |
...
2. `Map.tsx` reports the point to `page.tsx`; the page-owned query runner calls
   `getIncidents(...)` and annual radius history in parallel.
...
4. Crashes and annual route history are returned and displayed.

Uploaded routes are deliberately not bookmarkable in the version-1 URL
contract because their parsed GeoJSON can contain multiple features and
geometry types.

### Bookmarkable query URLs

`page.tsx` owns one execution boundary for radius, street, and manually drawn
route queries. Interactive actions and restored URLs both pass a validated
`QueryDefinitionV1` through that boundary, which dispatches to the existing
API clients. The APIs and PostGIS remain authoritative; URLs contain query
definitions, never crash results.

The browser URL describes the last query that completed successfully. Editing
draft controls does not change it, a failed replacement keeps the previous URL
and report, and successful zero-result queries are still copyable. The page
uses `router.replace` so repeated map clicks do not add browser-history entries.

Version 1 uses the following stable machine contract:

- every URL has `v=1`, `tool`, `from`, and `to`;
- radius queries add `lat`, `lng`, and `radiusFeet`;
- street queries add `street`, `bufferFeet`, and either both `crossFrom` and
  `crossTo` or neither; and
- drawn routes add `bufferFeet`, `precision=6`, and a Google encoded
  `polyline` generated by the standalone `@googlemaps/polyline-codec` package.

The parser rejects duplicate singleton parameters, unknown tools and fields,
non-canonical or reversed dates, out-of-range coordinates and distances,
unpaired cross streets, malformed routes, unavailable tools, and unsupported
versions. It applies no partial state and makes no crash request for an invalid
link. A complete legacy street URL using `fromDate`, `toDate`, `r`, `street`,
`crossStreet1`, and `crossStreet2` is accepted and replaced by its canonical
version-1 form only after the restored query succeeds.

Drawn routes are decoded into the same one-`LineString` `FeatureCollection`
used by interactive drawing. The conversion boundary swaps GeoJSON
`[longitude, latitude]` tuples to the codec's `[latitude, longitude]` order and
enforces valid coordinates, at least two distinct points, a 1,000-vertex decode
limit, and precision 6. The decoded route is the authoritative bookmarked
geometry.

The complete canonical path is limited to 2,000 characters after
`URLSearchParams` escaping. An interactive drawn route that exceeds that policy
still runs without simplification or truncation, but its query parameters are
removed and **Copy query link** is disabled with an explanation. The deployed
Nginx/Next.js path and canonical-host redirect still require release-time
verification at that exact limit.
