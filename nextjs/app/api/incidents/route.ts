...
export async function GET(request: NextRequest) {
  ...
  if (bbox) {
    ...
    whereClause += ` AND ST_Intersects(COALESCE(cdot.geo, doti.geo), ST_MakeEnvelope($${paramIndex++}, $${paramIndex++}, $${paramIndex++}, $${paramIndex++}, 4326))`;
    queryParams.push(minX, minY, maxX, maxY);
  } else if (lat && lng && radiusFeet) {
    const radiusInMeters = parseFloat(radiusFeet) * METERS_PER_FEET;
    whereClause += ` AND ST_DWithin(COALESCE(cdot.geo, doti.geo), ST_SetSRID(ST_Point($${paramIndex++}, $${paramIndex++}), 4326)::geography, $${paramIndex++})`;
